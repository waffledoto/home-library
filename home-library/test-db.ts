...
try {
  ...
  // Файл не найден - продолжим проверку
}
...
async function testConnection() {
  try {
    ...
    if (!url) {
      ...
      console.error('POSTGRES_URL=postgresql://postgres.xxx:PASSWORD@host:6543/postgres');
      process.exit(1);
    }
    ...
    // Проверка таблицы books
    const books = await db`SELECT COUNT(*) as count FROM books`;
    console.log('✅ Таблица books существует, книг:', books[0].count);
